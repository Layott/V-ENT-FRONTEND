...
const EventsHistory = () => {
    ...
    const currentTournaments = eventsList.slice(indexOfFirstTournament, indexOfLastTournament)
    ...
    const handleNextClick = () => {
        if (currentPage < Math.ceil(eventsList.length / rowsPerPage)) {
            setCurrentPage(currentPage + 1)
        }
    }
    ...
    // Pagination Controls
    const pageNumbers = []
        for (let i = 1; i <= Math.ceil(eventsList.length / rowsPerPage); i++) {
        pageNumbers.push(i)
    }
    ...
    <div className={profileStyles.tournamentEventsContainer}>
      ...
      <div className={styles.paginationContainer}>
        ...
        <div className={styles.pagination}>
          ...
          <button
            className={`${styles.navIconBTN} ${currentPage === Math.ceil(eventsList.length / rowsPerPage) ? styles.hidden : ''}`}
            onClick={handleNextClick}
            disabled={currentPage === Math.ceil(eventsList.length / rowsPerPage)}
          >
            <BsChevronRight
      ...
      <EventsDetails selectedTournament={selectedTournament} setSelectedTournament={setSelectedTournament} />
    </div>
  )
}
