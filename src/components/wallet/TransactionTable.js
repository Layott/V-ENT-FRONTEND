  ...
  return <div>
      {showFilters && <>
          ...
          <div className={styles.chipGroup}>
            {TYPE_FILTERS.map(t => <button key={t.id || 'all'} type="button" className={`${styles.chip} ${typeFilter === t.id ? styles.chipActive : ''}`} onClick={() => handleType(t.id)}>
                {byText(t.label)}
              </button>)}
          </div>
            ...
            <div className={styles.sectionControls}>
              ...
              <div className={styles.ctrlSelectWrap}>
                <select className={styles.ctrlSelect} value={statusFilter} onChange={e => handleStatus(e.target.value)} aria-label={tt("ui.status.filter.4cef", "Status filter")}>
                  {STATUS_OPTIONS.map(s => <option key={s.id || 'all'} value={s.id}>{byText(s.label)}</option>)}
                </select>
      ...
      <div className={styles.txTableWrap}>
        {loading ? <p className={styles.txLoading}>{tt("ui.loading.transactions.08d0", "Loading transactions…")}</p> : total === 0 ? <p className={styles.txEmpty}>{emptyText}</p> : <>
            <table className={styles.txTable}>
              <thead>
                <tr>
                  <th>{tt("ui.type.3deb", "Type")}</th>
                  <th>{tt("ui.description.55f8", "Description")}</th>
                  <th>{tt("ui.reference.db1c", "Reference")}</th>
              ...
              return <Fragment key={id}>
                      ...
                      {isOpen && <tr className={styles.txDetailRow}>
                          <td colSpan={6}>
                            <div className={styles.txDetailGrid}>
                              <div className={styles.txDetailItem}>
                                <span className={styles.txDetailLabel}>{tt("ui.reference.db1c", "Reference")}</span>
                                <span className={styles.txDetailValue}>{tx.reference || tx.ref || '-'}</span>
                              </div>
                              <div className={styles.txDetailItem}>
                                <span className={styles.txDetailLabel}>{tt("ui.submitted.2e00", "Submitted")}</span>
                              ...
                              {tx.balance_after != null && <div className={styles.txDetailItem}>
                                  <span className={styles.txDetailLabel}>{tt("ui.balance.after.50e7", "Balance after")}</span>
                                  <span className={styles.txDetailValue}>{formatNumber(tx.balance_after)} VC</span>
                                </div>}
                              {tx.amount_ngn != null && <div className={styles.txDetailItem}>
                                  <span className={styles.txDetailLabel}>{tt("ui.ngn.equivalent.739a", "NGN equivalent")}</span>
            ...
            return <div key={id} className={styles.txCard} onClick={() => toggleExpand(id)}>
                    ...
                    {isOpen && <div className={styles.txCardDetail}>
                        <div className={styles.txCardDetailRow}>
                          <span className={styles.txCardDetailKey}>{tt("ui.submitted.2e00", "Submitted")}</span>
                        ...
                        {tx.balance_after != null && <div className={styles.txCardDetailRow}>
                            <span className={styles.txCardDetailKey}>{tt("ui.balance.after.50e7", "Balance after")}</span>
                            <span className={styles.txCardDetailVal}>{formatNumber(tx.balance_after)} VC</span>
                          </div>}
                        {tx.amount_ngn != null && <div className={styles.txCardDetailRow}>
                            <span className={styles.txCardDetailKey}>{tt("ui.ngn.equivalent.739a", "NGN equivalent")}</span>
      ...
      {showPagination && total > 0 && <div className={styles.pagination}>
          <span className={styles.pagInfo}>
            {tt("ui.showing.163d", "Showing")} {start + 1}-{end} of {total}
          </span>
          <div className={styles.pagBtns}>
            <button className={styles.pagBtn} onClick={() => setPage(p => Math.max(p - 1, 1))} disabled={safePage === 1} aria-label={tt("ui.previous.page.81f5", "Previous page")}>
              <BsChevronLeft />
            </button>
            {Array.from({
          length: totalPages
        }, (_, i) => i + 1).map(n => <button key={n} className={`${styles.pagBtn} ${n === safePage ? styles.pagBtnActive : ''}`} onClick={() => setPage(n)}>
                {n}
              </button>)}
            <button className={styles.pagBtn} onClick={() => setPage(p => Math.min(p + 1, totalPages))} disabled={safePage === totalPages} aria-label={tt("ui.next.page.4bfc", "Next page")}>
              <BsChevronRight />
            </button>
          </div>
        </div>}
