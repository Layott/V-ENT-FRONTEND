...
const FIFAEvents = () => {
  ...
  useEffect(() => {
    const fetchFifaEvents = async () => {
      ...
      try {
        ...
        // Extract FIFA events from the by_game section
        const events = response.data.data.by_game["FIFA"] || [];
        setFifaEvents(events);
        console.log("Fetched FIFA events:", events);
      } catch (error) {
        console.error("Failed to fetch FIFA events:", error);
        if (error.response) {
  ...
  return <div className={allEventsStyles.fifaTournamentsContainer}>
            <div className={allEventsStyles.header}>
                <h3>{tt("ui.fifa.events.c0e9", "FIFA Events")}</h3>
                {!showAll && fifaEvents.length > 3 && <button className={allEventsStyles.seeMoreBTN} onClick={handleToggle}>
                        {tt("ui.see.more.c510", "See more")}<LuArrowRight />
                    </button>}
            </div>

            <div className={allEventsStyles.cardsContainer}>
                {fifaEvents.slice(0, showAll ? fifaEvents.length : 3).map((event, index) => <div key={index} className={allEventsStyles.cardContainer}>
                ...
                {showAll && fifaEvents.length > 3 && <button className={`${allEventsStyles.seeMoreBTN} ${allEventsStyles.seeLessBTN}`} onClick={handleToggle}>
                        <LuArrowLeft />{tt("ui.see.less.47c7", "See less")}
                    </button>}

                {fifaEvents.length === 0 && <div className={allEventsStyles.noEventsMessage}>
                        <p>{tt("ui.no.fifa.events.available.bb80", "No FIFA events available at the moment.")}</p>
                    </div>}
            </div>
        </div>;
};
