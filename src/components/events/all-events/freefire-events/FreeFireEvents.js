...
const FreeFireEvents = () => {
  ...
  return <div className={allEventsStyles.fifaTournamentsContainer}>
            <div className={allEventsStyles.header}>
                <h3>{tt("ui.free.fire.events.5f11", "Free Fire Events")}</h3>
                {!showAll && freeFireEvents.length > 3 && <button className={allEventsStyles.seeMoreBTN} onClick={handleToggle}>
        ...
        return <div key={eventId} className={allEventsStyles.cardContainer}>
                ...
                {showAll && freeFireEvents.length > 3 && <button className={`${allEventsStyles.seeMoreBTN} ${allEventsStyles.seeLessBTN}`} onClick={handleToggle}>
                        <LuArrowLeft />{tt("ui.see.less.47c7", "See less")}
                    </button>}

                {freeFireEvents.length === 0 && <div className={allEventsStyles.noEventsMessage}>
                        <p>{tt("ui.no.free.fire.events.416d", "No Free Fire events available at the moment.")}</p>
                    </div>}
            </div>
        </div>;
};
