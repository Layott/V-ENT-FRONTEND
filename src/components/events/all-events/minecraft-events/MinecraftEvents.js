...
const MinecraftEvents = () => {
  ...
  return <div className={allEventsStyles.fifaTournamentsContainer}>
            <div className={allEventsStyles.header}>
                <h3>{tt("ui.minecraft.events.75bb", "Minecraft Events")}</h3>
                {!showAll && minecraftEvents.length > 3 && <button className={allEventsStyles.seeMoreBTN} onClick={handleToggle}>
                        {tt("ui.see.more.c510", "See more")}<LuArrowRight />
                    </button>}
            </div>

            <div className={allEventsStyles.cardsContainer}>
                {minecraftEvents.slice(0, showAll ? minecraftEvents.length : 3).map((event, index) => <div key={index} className={allEventsStyles.cardContainer}>
                ...
                {showAll && minecraftEvents.length > 3 && <button className={`${allEventsStyles.seeMoreBTN} ${allEventsStyles.seeLessBTN}`} onClick={handleToggle}>
                        <LuArrowLeft />{tt("ui.see.less.47c7", "See less")}
                    </button>}

                {minecraftEvents.length === 0 && <div className={allEventsStyles.noEventsMessage}>
                        <p>{tt("ui.no.minecraft.events.available.fe7c", "No Minecraft events available at the moment.")}</p>
                    </div>}
            </div>
        </div>;
};
