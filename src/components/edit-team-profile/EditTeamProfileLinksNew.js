...
const FIELDS = [{
  ...
  label: 'Twitch',
  icon: FaTwitch,
  placeholder: 'https://twitch.tv/team'
}, {
  key: 'youtube',
  label: 'YouTube',
  icon: FaYoutube,
...
const EditTeamProfileLinksNew = ({
  ...
  return <form className={styles.formWrapper} onSubmit={handleSave}>
      <h2 className={styles.sectionTitle}>{tt("ui.web.social.links.d9e8", "Web and Social Links")}</h2>
      <p className={styles.helperText}>{tt("ui.add.any.team.s.fbe5", "Add any of your team’s public profiles. Leave blank to hide.")}</p>
      ...
      placeholder
          ...
          <input type="url" value={links[key]} onChange={e => setLinks(s => ({
        ...s,
        [key]: e.target.value
