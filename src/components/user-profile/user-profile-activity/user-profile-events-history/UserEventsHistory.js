...
const UserEventsHistory = () => {
    ...
    const currentTournaments = userEventsList.slice(indexOfFirstTournament, indexOfLastTournament)
    ...
    const handleNextClick = () => {
        if (currentPage < Math.ceil(userEventsList.length / rowsPerPage)) {
            setCurrentPage(currentPage + 1)
        }
    }
    ...
    // Pagination Controls
    const pageNumbers = []
        for (let i = 1; i <= Math.ceil(userEventsList.length / rowsPerPage); i++) {
        pageNumbers.push(i)
    }
    ...
    <div className={profileStyles.tournamentEventsContainer}>
        <div className={profileStyles.tournamentsEventsFilterSearchContainer}>
            <div className={profileStyles.tournamentsEventsFilterContainer}>
                <p className={styles.tournamentNumber}>{userEventsList.length} events</p>
      ...
      <div className={styles.paginationContainer}>
        ...
        <p className={profileStyles.showingNumber}>
          ...
          {indexOfLastTournament > userEventsList.length ? userEventsList.length : indexOfLastTournament}{" "} of {userEventsList.length}
        </p>
        ...
        <div className={styles.pagination}>
          ...
          <button
            className={`${styles.navIconBTN} ${currentPage === Math.ceil(userEventsList.length / rowsPerPage) ? styles.hidden : ''}`}
            onClick={handleNextClick}
            disabled={currentPage === Math.ceil(userEventsList.length / rowsPerPage)}
          >
            <BsChevronRight
