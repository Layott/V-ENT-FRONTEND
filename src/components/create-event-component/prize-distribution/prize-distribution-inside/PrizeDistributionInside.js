      ...
      <div className={createTournamentStyles.innerCreateSubSectionContainer}>
        ...
        {selectedOption === "distributed" && <div className={styles.distributedContainer}>
            <p className={createTournamentStyles.infoParagraph}>
              ...
              {tt("ui.any.position.left.empty.e02e", "Any position left empty will be omitted. All amounts are in v-ent coins.")}
            </p>
            ...
            {positions.map(position => <div key={position} className={`${createTournamentStyles.twoInputContainer} ${styles.twoInputContainer}`}>
                <div className={createTournamentStyles.inputGroup}>
                  ...
                  <input id={`prizePosition${position}`} type="number" placeholder={`Enter Prize for ${position} Place (Number)`} className={createTournamentStyles.inputNumber} value={prizes[`prizePosition${position}`] || ""} onChange={e => handlePrizeChange(position, e.target.value)} />
                </div>
                ...
                <div className={createTournamentStyles.inputGroup}>
                  ...
                  <input id={`extraBonus${position}`} type="text" placeholder={tt("ui.enter.extra.bonus.text.8d12", "Enter Extra Bonus (Text)")} className={`${createTournamentStyles.inputText} ${styles.inputText}`} value={extraBonuses[`extraBonus${position}`] || ""} onChange={e => handleBonusChange(position, e.target.value)} />
                </div>
        ...
        {selectedOption === "winner-takes-all" && <div className={styles.winnerTakesAllContainer}>
            <div className={`${createTournamentStyles.inputGroup} ${styles.inputGroup}`}>
              ...
              <input id="winnerPrize" type="number" placeholder={tt("ui.enter.amount.prize.ea29", "Enter Amount for Prize")} className={createTournamentStyles.inputNumber} />
            </div>
            <p className={createTournamentStyles.infoParagraph}>
              <span className={styles.infoSpan}>
                <FiInfo className={styles.infoIcon} />
        ...
        {selectedOption === "distributed" && totalPrize > 0 && <div className={styles.totalPrizeContainer}>
            <p className={styles.totalPrizeText}>
              {tt("ui.total.prize.f0d2", "Total Prize:")} <span className={styles.totalPrizeAmount}>{formatNumber(totalPrize)} {tt("ui.v.ent.coins.9ef4", "v-ent coins")}</span>
            </p>
          </div>}
      </div>
    </div>;
