...
const Avatar = ({
  ...
  if (!src || errored) {
    return <div className={styles.avatarFallback}>
        {initials}
      </div>;
  }
  return (
    // eslint-disable-next-line @next/next/no-img-element
    <img src={src} alt={name} className={tableStyles.gameImage} onError={() => setErrored(true)} />
  );
};
const TournamentDetailsParticipants = ({
  ...
  const fetchParticipants = async () => {
    ...
    try {
      ...
      setError(tt("msg.failedToLoadParticipants", "Failed to load participants."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const ParticipantRow = ({
    ...
    return <div className={expanded ? `${styles.gridRowExpanded} ${tableStyles.gridRowExpanded} ${profileStyles.middleLayerColor}` : `${styles.gridRow} ${tableStyles.gridRow} ${profileStyles.middleLayerColor}`}>
        ...
        <div className={`${item()} ${tableStyles.viewProfileBTNContainer}`}>
          <Link href={participant.profileId ? `/u/${participant.profileId}` : '/user-profile'} className={`${tableStyles.viewProfileBTN} ${profileStyles.topMostLayerColor}`}>
  ...
  return <div className={tableStyles.tournamentDetailsParticipantsContainer}>
      <div className={tableStyles.informationArea}>
        ...
        <p>{tt("ui.teams.individuals.can.register.f0f7", "Teams and Individuals can register for this tournament.")}</p>
      </div>
      ...
      <div className={tableStyles.tournamentsEventsFilterSearchContainer}>
        ...
        <div className={tableStyles.searchContainer}>
          <div className={tableStyles.searchBar}>
            ...
            <input type="text" placeholder={tt("ui.search.participants.82d5", "Search participants")} className={tableStyles.searchInput} value={searchQuery} onChange={e => {
      ...
      {loading && <p className={styles.stateText}>{tt("ui.loading.participants.b19e", "Loading participants...")}</p>}
      {error && <p className={styles.errorText}>{error}</p>}

      {!loading && !error && filtered.length === 0 && <p className={styles.stateText}>
          {searchQuery ? tx("No participants match your search.") : tx("No participants have registered yet.")}
        </p>}
      ...
      {!loading && !error && filtered.length > 0 && <>
          {!showGalleryView ? <div className={`${tableStyles.participantsTable} ${styles.participantsTable}`}>
              ...
              <div className={tableStyles.participantsTableExpanded}>
                <div className={`${styles.gridHeaderExpanded} ${tableStyles.gridHeaderExpanded} ${profileStyles.middleLayerColor}`}>
                  {headerCols.map(col => <div key={col} className={`${tableStyles.gridItemExpanded} ${tableStyles.gridItemHeaderExpanded}`}>{col}</div>)}
                </div>
                {currentParticipants.map((p, i) => <ParticipantRow key={p.id ?? i} participant={p} expanded={true} />)}
              </div>
            </div>}
          ...
          <div className={tableStyles.paginationContainer}>
            ...
            <p className={profileStyles.showingNumber}>
              {tt("ui.showing.163d", "Showing")} {indexOfFirst + 1}-{Math.min(indexOfLast, filtered.length)} of {filtered.length}
            </p>
            ...
            <div className={tableStyles.pagination}>
              ...
              {pageNumbers.map(n => <button key={n} className={`${tableStyles.pageBTN} ${currentPage === n ? tableStyles.activePage : ''}`} onClick={() => handlePageChange(n)}>
                  {n}
                </button>)}
              <button className={`${tableStyles.navIconBTN} ${currentPage === totalPages ? tableStyles.hidden : ''}`} onClick={handleNextClick} disabled={currentPage === totalPages}>
