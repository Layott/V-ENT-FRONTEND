...
const ChooseTeamModal = ({ isOpen, onClose, onProceed, onBack }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const data = await ventFetch(API.TEAM.MY_TEAMS, { token });
  ...
  const filteredTeams = useMemo(() => {
    ...
    return teams.filter((team) =>
      team.name.toLowerCase().includes(q) || (team.game || '').toLowerCase().includes(q)
    );
  }, [teams, searchTerm]);
      ...
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalBody}>
          ...
          <div className={styles.teamsHeader}>
            <span className={styles.teamsCount}>{teams.length} team{teams.length !== 1 ? 's' : ''}</span>
          ...
          <div className={styles.teamsContainer}>
            {loading ? (
              <p className={styles.subtitle} style={{ textAlign: 'center', padding: '1.5rem 0' }}>Loading your teams…</p>
            ) : error ? (
              <p className={styles.subtitle} style={{ textAlign: 'center', padding: '1.5rem 0' }}>{error}</p>
            ) : filteredTeams.length === 0 ? (
              <p className={styles.subtitle} style={{ textAlign: 'center', padding: '1.5rem 0' }}>
                {teams.length === 0
                  ? "You're not in any teams yet. Create a team to register as a team."
                ...
                >
                  <div className={styles.teamInfo}>
                    <div className={styles.teamImage}>
                      ...
                      <img
                        src={team.logo || fallbackLogo.src}
                        alt={team.name}
                        width={60}
                        height={60}
