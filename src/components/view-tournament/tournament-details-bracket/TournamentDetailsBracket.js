...
function WinnerTrophy() {
  const tt = useT();
  return <span className={styles.winnerBadge} aria-label={tt("ui.winner.4e3a", "Winner")}>
...
function MatchCard({
  ...
  function scoreEl(participant, opponent) {
    ...
    return <span className={`${styles.score} ${isWinner ? styles.scoreWinner : ''} ${isLoser ? styles.scoreLoser : ''}`}>
        {participant?.score ?? '-'}
      </span>;
  }
  ...
  return <div className={`${styles.matchCard} ${cardMod}`} style={{
    top: topY
  }}>
      {/* Card top row */}
      <div className={styles.cardTop}>
        {match.match_number && <span className={styles.matchLabel}>{tt("ui.match.0335", "Match")} {match.match_number}</span>}
...
function BracketConnector({
  ...
  return <svg className={styles.connectorSvg} width={CONNECTOR_W} height={totalHeight} xmlns="http://www.w3.org/2000/svg">
      {lines}
    </svg>;
}
...
const TournamentDetailsBracket = ({
  tournament
}) => {
  const tx = useTx();
  const tt = useT();
  ...
  if (loading) {
    return <div className={styles.stateContainer}>
        <span className={styles.stateText}>{tt("ui.loading.bracket.8c4e", "Loading bracket…")}</span>
      </div>;
  }
  if (error) {
    return <div className={styles.stateContainer}>
        <span className={styles.errorText}>{error}</span>
        <button className="btn goldBTN" onClick={fetchBracket}>{tt("ui.retry.9f5c", "Retry")}</button>
      </div>;
  }
  if (!rounds.length) {
    return <div className={styles.stateContainer}>
        <span className={styles.stateText}>{tt("ui.bracket.has.not.been.0e32", "Bracket has not been generated yet.")}</span>
      </div>;
  }
  return <div className={styles.bracketWrapper}>
      {bracketType && <p className={styles.bracketType}>
          {bracketType === 'single_elimination' ? tx("Single Elimination") : bracketType}
        </p>}
      ...
      <div className={styles.bracketScroll}>
        <div className={styles.bracketInner}>
          ...
          return <div key={round.id || roundIdx} className={styles.roundGroup}>
                {/* Round column */}
                <div className={styles.roundCol}>
                  <div className={styles.roundHeader}>
                    ...
                    {allDone && <span className={styles.roundBadge}>{tt("ui.completed.1798", "Completed")}</span>}
                  </div>

                  <div className={styles.cardsArea} style={{
                height: bracketHeight
              }}>
                    {(round.matches || []).map((match, matchIdx) => <MatchCard key={match.id || matchIdx} match={match} topY={positions[matchIdx] ?? 0} />)}
                  </div>
                </div>

                {/* Connector to next round */}
                {!isLast && allPositions[roundIdx + 1] && <div className={styles.connectorCol}>
                    <div className={styles.connectorSpacer} />
                    <BracketConnector prevPositions={positions} nextPositions={allPositions[roundIdx + 1]} totalHeight={bracketHeight} />
                  </div>}
              </div>;
        })}
        </div>
