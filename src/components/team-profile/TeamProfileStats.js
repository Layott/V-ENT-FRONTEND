...
const TeamProfileStats = ({ team }) => {
  ...
  const fillD = points.length
    ? `${pathD} L ${points[points.length - 1][0]} ${h - padY} L ${points[0][0]} ${h - padY} Z`
    : ''
    ...
    <div className={styles.statsContainer}>
      ...
      <div className={styles.kpiGrid}>
        ...
        <div className={`${styles.kpiCard} ${styles.kpiAccentRed}`}>
          ...
          <div>
            ...
            <p className={styles.kpiLabel}>Tournament wins</p>
          </div>
        </div>
        <div className={styles.kpiCard}>
          ...
          <div>
            ...
            <p className={styles.kpiLabel}>W / L record</p>
          </div>
        </div>
        <div className={styles.kpiCard}>
          ...
          <div>
            ...
            <p className={styles.kpiLabel}>Total prize</p>
          </div>
        </div>
        <div className={styles.kpiCard}>
          ...
          <div>
            ...
            <p className={styles.kpiLabel}>Global rank</p>
          </div>
        </div>
      </div>
      ...
      <div className={styles.statsCharts}>
        ...
        <section className={styles.panel}>
          <h3 className={styles.panelTitle}>Win-rate over time</h3>
          {series.length === 0 ? (
            <p className={styles.stateText}>No history yet.</p>
          ) : (
            <div className={styles.sparkline}>
              <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className={styles.sparkSvg}>
                ...
                {points.map(([x, y], i) => (
                  <circle key={i} cx={x} cy={y} r="3" fill="var(--v-ent-gold)" stroke="#131316" strokeWidth="1" />
                ))}
              </svg>
              <div className={styles.sparkLabels}>
                {series.map((pt, i) => (
                  <span key={i} className={styles.sparkLabel}>{pt.month}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <section className={styles.panel}>
          <h3 className={styles.panelTitle}>Most-played games</h3>
          {games.length === 0 ? (
            <p className={styles.stateText}>No data yet.</p>
          ) : (
            <ul className={styles.gameBreakdown}>
              {games.map((g) => (
                <li key={g.game} className={styles.gameRow}>
                  <span className={styles.gameName}>{g.game}</span>
                  <div className={styles.gameBar}>
                    <div className={styles.gameBarFill} style={{ width: `${(g.count / maxGames) * 100}%` }} />
                  </div>
