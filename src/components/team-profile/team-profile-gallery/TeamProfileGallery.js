...
import styles from './team-profile-gallery.module.css';
...
const TeamProfileGallery = () => {
  ...
  return <div className={styles.galleryContainer}>
      ...
      display: 'none'
        ...
        <div className={`${styles.imageContainer} ${styles.addImageBTN}`} onClick={handleUploadClick}>
            <div className={`${styles.addImage} ${profileStyles.topMostLayerColor}`}>
                <span className={styles.plusIcon}><FiPlus /></span>
                <span className={styles.uploadImageText}>{tt("ui.upload.image.1590", "Upload Image")}</span>
            </div>
        </div>

        {galleryData.map((gallery, index) => <div key={index} className={styles.imageContainer}>
                <Image src={gallery.src} alt={gallery.name} />
            </div>)}
    </div>;
};
export default TeamProfileGallery;