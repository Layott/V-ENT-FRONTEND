...
const TeamProfileTournamentsHistory = ({ teamId }) => {
        ...
        const fetchTournaments = async () => {
            try {
                ...
                const response = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL}/team/tournaments/${teamId}/`,
                    { method: 'GET', headers }
                );
    ...
    const handleNextClick = () => { if (currentPage < Math.ceil(tournaments.length / rowsPerPage)) setCurrentPage(currentPage + 1) }
    ...
    <div className={profileStyles.tournamentEventsContainer}>
        <div className={profileStyles.tournamentsEventsFilterSearchContainer}>
            <div className={profileStyles.tournamentsEventsFilterContainer}>
                <p className={styles.tournamentNumber}>{tournaments.length} tournaments</p>
      ...
      <div className={`${profileStyles.tournamentsEventsTable}`}>
        ...
        {tournaments.length === 0 && <p>No tournament history.</p>}

        {currentTournaments.map((tournament, index) => {
          const logoUrl = getImageUrl(tournament.tournament_logo || tournament.logo || tournament.src);
          return (
            <div key={tournament.id || tournament.tournament_id || index} className={`${styles.gridRow} ${profileStyles.middleLayerColor}`}>
              <div className={`${styles.gridItem} ${styles.nameColumn}`}>
                <div className={styles.gameImageContainer}>
                  {logoUrl ? (
                    <Image src={logoUrl} alt={tournament.tournament_title || tournament.name || ''} className={styles.gameImage} width={36} height={36} />
                  ) : (
      ...
      <div className={styles.paginationContainer}>
        ...
        <div className={styles.pagination}>
          ...
          <button
            className={`${styles.navIconBTN} ${currentPage === Math.ceil(tournaments.length / rowsPerPage) ? styles.hidden : ''}`}
            onClick={handleNextClick}
            disabled={currentPage === Math.ceil(tournaments.length / rowsPerPage)}
          >
