...
const TeamProfileRequests = ({ team, onToast }) => {
    ...
    const load = async () => {
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/team/join-requests/${team.id}/`,
          { headers }
        )
  ...
  return (
    <div className={styles.requestsContainer}>
      <section className={styles.panel}>
        <div className={styles.tableHeader}>
          <h3 className={styles.panelTitle}>
            Pending join requests <span className={styles.countPill}>{requests.length}</span>
          </h3>
        </div>

        {requests.length === 0 ? (
          <p className={styles.stateText}>No pending requests.</p>
        ) : (
              ...
              <div key={r.id} className={styles.requestCard}>
                <div className={styles.memberCell}>
                  <div className={styles.memberAvatar}>
                    {r.applicant?.avatar ? (
                      <Image src={r.applicant.avatar} alt="" width={48} height={48} />
                    ) : (
                  ...
                  <div>
                    ...
                    <p className={styles.memberHandle}>@{r.applicant?.username} · Rank #{r.applicant?.rank || '-'}</p>
                  </div>
                </div>
      ...
      <section className={styles.panel}>
        <div className={styles.tableHeader}>
          <h3 className={styles.panelTitle}>
            Invites sent <span className={styles.countPill}>{invites.length}</span>
          </h3>
        </div>

        {invites.length === 0 ? (
          <p className={styles.stateText}>No outgoing invites.</p>
        ) : (
          <div className={styles.requestList}>
            {invites.map((i) => (
              <div key={i.id} className={styles.requestCard}>
                <div className={styles.memberCell}>
                  <div className={styles.memberAvatar}>
                    {i.invited_by?.avatar ? (
                      <Image src={i.invited_by.avatar} alt="" width={48} height={48} />
                    ) : (
                      <div className={styles.avatarFallback} />
                    )}
                  </div>
                  <div>
                    ...
                    <p className={styles.memberHandle}>Role offered: {i.role}</p>
                  </div>
                </div>
                <p className={styles.requestMessage}>{i.message}</p>
                <div className={styles.requestFooter}>
                  <span className={styles.requestTime}><LuClock /> Sent {fmtDate(i.created_at)}</span>
