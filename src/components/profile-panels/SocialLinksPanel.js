...
const Icon = ({
  ...
  if (kind === 'instagram') return <svg viewBox="0 0 24 24" fill="none">
      <defs>
...
const SocialLinksPanel = ({
  ...
  return <div className={styles.wrap}>
      <div className={styles.card}>
        ...
        {socialLinks.length === 0 ? <p className={styles.empty}>{tt("ui.no.social.links.added.0edb", "No social links added.")}</p> : <div className={styles.grid}>
            {socialLinks.map((s, idx) => <a key={idx} href={s.url} target="_blank" rel="noreferrer noopener" className={styles.pill}>
