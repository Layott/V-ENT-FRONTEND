...
const SocialIcon = ({ kind }) => {
  const map = {
    facebook: (
      <svg viewBox="0 0 24 24" fill="#1877F2"><path d="M22 12a10 10 0 1 0-11.6 9.9v-7H7.9V12h2.5V9.8c0-2.5 1.5-3.9 3.8-3.9 1.1 0 2.2.2 2.2.2v2.5h-1.3c-1.2 0-1.6.8-1.6 1.5V12h2.8l-.5 2.9h-2.3v7A10 10 0 0 0 22 12z"/></svg>
    ),
    instagram: (
    ...
    x: (
      <svg viewBox="0 0 24 24" fill="#fff"><path d="M18 3h3l-7.5 8.6L22 21h-6.5l-5-6.5L4.8 21H1.7l8-9.2L1 3h6.6l4.5 6 5.9-6z"/></svg>
    ),
    twitter: (
      <svg viewBox="0 0 24 24" fill="#fff"><path d="M18 3h3l-7.5 8.6L22 21h-6.5l-5-6.5L4.8 21H1.7l8-9.2L1 3h6.6l4.5 6 5.9-6z"/></svg>
    ),
    ...
    twitch: (
      <svg viewBox="0 0 24 24" fill="#9146FF"><path d="M4 2v18h6v3h3l3-3h5l5-5V2H4zm17 11l-3 3h-5l-3 3v-3H6V4h15v9zm-3-7v6h-2V6h2zm-5 0v6h-2V6h2z"/></svg>
    ),
  };
    ...
    <div className={styles.overviewGrid}>
      ...
      <div className={styles.leftCol}>
        ...
        <div className={styles.card}>
          ...
          {interests.length === 0 ? (
            <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.82rem', fontFamily: 'Inter' }}>
              No interests added yet.
            </p>
          ) : (
            <div className={styles.chipRow}>
              {visibleInterests.map((it, idx) => (
                <span key={idx} className={styles.chip}>{it}</span>
              ))}
              {remainingInterests > 0 && (
                <button type="button" className={styles.chipMore}>See more · +{remainingInterests}</button>
              )}
            </div>
          )}
        </div>
        ...
        <div className={styles.card}>
          <div className={styles.cardHead}>
            <h3 className={styles.cardTitle}>Gaming Accounts</h3>
          </div>
          {gamingAccounts.length === 0 ? (
            <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.82rem', fontFamily: 'Inter' }}>
              No gaming accounts linked.
            </p>
          ) : (
            <div className={styles.acctList}>
              {gamingAccounts.slice(0, 4).map((a, idx) => {
                ...
                return (
                  <div className={styles.acctRow} key={idx}>
                    <div className={styles.acctLogo} style={a.color ? { backgroundColor: a.color } : undefined}>
                      {a.icon ? <img src={a.icon} alt="" /> : initials}
                    </div>
              ...
              {gamingAccounts.length > 4 && (
                <button type="button" className={styles.chipMore}>See more · +{gamingAccounts.length - 4}</button>
              )}
            </div>
          )}
        </div>
          ...
          {socialLinks.length === 0 ? (
            ...
            <div className={styles.socialGrid}>
              {socialLinks.slice(0, 6).map((s, idx) => {
                ...
                return (
                  <a className={styles.socialPill} key={idx} href={s.url} target="_blank" rel="noreferrer noopener">
      ...
      <div className={styles.rightCol}>
        ...
        <div className={styles.statStrip}>
          <div className={styles.statCard}>
            <div className={styles.statCardHead}>Wallet Balance</div>
          ...
          <div className={styles.statCard}>
            <div className={styles.statCardHead}>Penalty Points</div>
            <div className={styles.statAmountRow}>
          ...
          <div className={styles.statsGrid}>
            <div className={styles.statCell}>
              <div className={styles.statCellHead}>
                <span className={styles.statCellLabel}>Ranking</span>
                <button type="button" className={styles.statCellLink}>View Table</button>
              </div>
              <div className={styles.statCellValue}>{rank ? `#${rank}` : '-'}</div>
            </div>
            <div className={styles.statCell}>
              <div className={styles.statCellHead}>
                <span className={styles.statCellLabel}>Tournaments played</span>
              </div>
              <div className={styles.statCellValue}>{tournamentsPlayed}</div>
            </div>
            <div className={styles.statCell}>
              <div className={styles.statCellHead}>
                <span className={styles.statCellLabel}>Wins</span>
              </div>
              <div className={styles.statCellValue}>{wins}</div>
            </div>
            <div className={styles.statCell}>
              <div className={styles.statCellHead}>
                <span className={styles.statCellLabel}>Losses</span>
              </div>
        ...
        <div className={styles.card}>
          <div className={styles.cardHead}>
            ...
            {favoriteGames.length > 0 && (
              <button type="button" className={styles.cardLink} onClick={onSeeAll}>
                See more
          ...
          <div className={styles.gamesGrid}>
            {isOwner && (
              <div className={styles.gameTile} onClick={onAddGame} role="button" tabIndex={0}>
                ...
                <span className={styles.gameName}>Add Game</span>
              </div>
            )}
            {visibleGames.map((g, idx) => (
              <div className={styles.gameTile} key={g.id || g.name || idx}>
                <div className={styles.gameImg}>
                  {g.cover ? <img src={g.cover} alt={g.name || g.title || ''} /> : null}
                </div>
        ...
        <div className={styles.card}>
          ...
          {achievements.length === 0 ? (
            <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.82rem', fontFamily: 'Inter', textAlign: 'center', padding: '1.5rem 0' }}>
              No achievements yet.
            </p>
          ) : (
            <div className={styles.achvGrid}>
              {achievements.slice(0, 6).map((a, idx) => (
                <div className={styles.achvTile} key={a.id || idx}>
                  <div className={styles.achvMedal}>
                    {a.icon ? (
                      <img src={a.icon} alt={a.title || ''} style={{ maxWidth: '85%', maxHeight: '85%', objectFit: 'contain' }} />
                    ) : (
                      <svg viewBox="0 0 64 80" style={{ width: '75%', height: '75%' }}>
