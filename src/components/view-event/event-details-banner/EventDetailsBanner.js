...
const EventDetailsBanner = ({
  ...
  if (!event) {
    ...
    return <div className={bannerDetailsStyles.tournamentDetailsBannerContainer}>
        <div className={bannerDetailsStyles.tournamentDetailsBanner}>
          <Image src={mediaUrl(tournamentDetailsBanner)} alt={tt("ui.event.details.banner.9c6c", "Event Details Banner")} fill style={{
          objectFit: 'cover'
        }} />
        </div>
        <div className={bannerDetailsStyles.headerContainer}>
          <div className={bannerDetailsStyles.headerLeft}>
            <h2>{tt("ui.loading.event.details.e416", "Loading event details...")}</h2>
            <div className={bannerDetailsStyles.headerDetails}>
              <p className={bannerDetailsStyles.tournamentStatus}>{tt("ui.loading.b04b", "Loading...")}</p>
              <p className={bannerDetailsStyles.tournamentTimeRemaining}>
                <LuClock3 className={bannerDetailsStyles.clockIcon} /> {tt("ui.loading.b04b", "Loading...")}
              </p>
              <p className={bannerDetailsStyles.tournamentFee}>
                <PiMoneyWavy className={bannerDetailsStyles.moneyIcon} /> {tt("ui.entry.fee.bf4f", "Entry Fee:")} 
                <span className={bannerDetailsStyles.feeSpan}>{tt("ui.loading.b04b", "Loading...")}</span>
              </p>
            </div>
          </div>
          <div className={bannerDetailsStyles.headerRight}>
            ...
            <div className={bannerDetailsStyles.joinTournamentBTN} style={{
            opacity: 0.5
          }}>
              {tt("ui.loading.b04b", "Loading...")}
            </div>
          </div>
        </div>
      </div>;
  ...
  return <div className={bannerDetailsStyles.tournamentDetailsBannerContainer}>
      <div className={bannerDetailsStyles.tournamentDetailsBanner}>
        <Image src={mediaUrl(bannerUrl)} alt={event.name ? `${event.name} Banner` : tx("Event Details Banner")} fill style={{
        objectFit: 'cover'
      }} onError={e => {
      ...
      <div className={bannerDetailsStyles.headerContainer}>
        <div className={bannerDetailsStyles.headerLeft}>
          <h2>{event.name || tx("Event Name")}</h2>
          <div className={bannerDetailsStyles.headerDetails}>
        ...
        <div className={bannerDetailsStyles.headerRight}>
          ...
          {onBookNow && <button onClick={onBookNow} className={`${bannerDetailsStyles.joinTournamentBTN} goldBTN`} style={{
          border: 'none',
          cursor: 'pointer'
        }} type="button">
