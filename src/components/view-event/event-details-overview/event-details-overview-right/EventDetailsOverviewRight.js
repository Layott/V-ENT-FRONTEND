...
const EventDetailsOverviewRight = ({
  ...
  const copyToClipboard = text => {
    ...
    if (!isClipboardAvailable) {
      setMessage(tt("msg.clipboardApiIsNotAvailable", "Clipboard API is not available!"));
      setSeverity("error");
      setOpen(true);
      return;
    }
    navigator.clipboard.writeText(text).then(() => {
      setMessage(tt("msg.linkCopiedToClipboard", "Link copied to clipboard!"));
      setSeverity("success");
      setOpen(true);
    }).catch(() => {
      setMessage(tt("msg.failedToCopyTheLink", "Failed to copy the link!"));
      setSeverity("error");
      setOpen(true);
    });
  };
  ...
  return <div className={overviewLtStyles.overviewRight}>
      <div className={overviewRtStyles.rightBox}>
        <div className={overviewRtStyles.paragraphDiv}>
          <p className={overviewRtStyles.paragraphTitle}>
            ...
            {tt("ui.event.type.2e1d", "Event Type")}
          </p>
          <p className={overviewRtStyles.paragraphValue}>
            {event?.event_type || tx("Not specified")}
          </p>
        </div>
        ...
        <div className={overviewRtStyles.paragraphDiv}>
          <p className={overviewRtStyles.paragraphTitle}>
            ...
            {tt("ui.entry.fee.a428", "Entry Fee")}
          </p>
          <p className={overviewRtStyles.paragraphValue}>
        ...
        <div className={overviewRtStyles.paragraphDiv}>
          ...
          <p className={overviewRtStyles.paragraphValue}>
            {event?.game || tx("Game not specified")}
          </p>
        </div>
      ...
      <div className={overviewRtStyles.rightBox}>
        <h3 className={`${overviewRtStyles.headerH3} ${tournamentStyles.headerH3}`}>
          <FaLink className={overviewRtStyles.priceIcon} /> {tt("ui.event.link.46a2", "Event Link")}
        </h3>
        
        <div className={overviewRtStyles.paragraphDiv}>
          <p className={overviewRtStyles.paragraphTitle}>{tt("ui.platform.123a", "Platform")} </p>
          <p className={`${overviewRtStyles.paragraphValue} ${overviewRtStyles.paragraphValueFloatRight}`}>
            {linkText ? getPlatform(linkText) : tx("Not set")}
          </p>
        </div>

        {linkText ? <div className={overviewRtStyles.linkDiv}>
            <p className={overviewRtStyles.linkToBeCopied}>{linkText}</p>
            <button type="button" className={overviewRtStyles.copyBTN} onClick={() => copyToClipboard(linkText)}>
              <FiCopy className={`${overviewRtStyles.icons} ${overviewRtStyles.strokeIcons}`} />
              <span className={overviewRtStyles.copySpan}>{tt("ui.copy.af74", "Copy")}</span>
            </button>
          </div> : <p className={overviewRtStyles.paragraphValue}>
            {tt("ui.organiser.has.not.added.6936", "The organiser has not added a link for this event yet.")}
          </p>}
        ...
        <div className={overviewRtStyles.linkDiv}>
          <p className={overviewRtStyles.linkToBeCopied}>{tt("ui.share.event.589d", "Share this event")}</p>
          <button type="button" className={overviewRtStyles.copyBTN} onClick={shareEvent}>
            <FiCopy className={`${overviewRtStyles.icons} ${overviewRtStyles.strokeIcons}`} />
            <span className={overviewRtStyles.copySpan}>{tt("ui.share.09ca", "Share")}</span>
          </button>
        </div>
      </div>
      ...
      <div className={`${overviewRtStyles.rightBox} ${overviewRtStyles.removeBorderBottom}`}>
        <h3 className={`${overviewRtStyles.headerH3} ${tournamentStyles.headerH3}`}>
          <BiMapPin className={overviewRtStyles.priceIcon} /> {tt("ui.direction.fd8e", "Direction")}
        </h3>

        <div className={overviewRtStyles.paragraphDiv}>
          <p className={overviewRtStyles.paragraphTitle}>
            <IoLocationOutline className={overviewRtStyles.icons} /> {tt("ui.address.d70f", "Address")} 
          </p>
          <p className={overviewRtStyles.paragraphValue}>{address}</p>
        </div>

        <div className={overviewRtStyles.linkDiv}>
          <button className={overviewRtStyles.getDirectionBTN} onClick={() => {
          const googleMapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`;
          window.open(googleMapsUrl, '_blank');
        }}>
            {tt("ui.get.directions.0e16", "Get Directions")}
          </button>
          <button className={overviewRtStyles.copyBTN} onClick={() => copyToClipboard(address)}>
            <FiCopy className={`${overviewRtStyles.icons} ${overviewRtStyles.strokeIcons}`} />
            <span className={overviewRtStyles.copySpan}>{tt("ui.copy.af74", "Copy")}</span>
          </button>
        </div>
        ...
        <button onClick={toggleMapVisibility} className={overviewRtStyles.hideMapBTN}>
          {isMapVisible ? tx("Hide Map") : tx("Show Map")}{" "}
          {isMapVisible ? <PiCaretUpBold className={overviewRtStyles.caretIcons} /> : <PiCaretDownBold className={overviewRtStyles.caretIcons} />}
        </button>
      </div>

      {isMapVisible && address && address !== "Event location not specified" && <div className={overviewRtStyles.mapContainer}>
          <iframe src={`https://www.google.com/maps/embed/v1/place?key=YOUR_GOOGLE_MAPS_API_KEY&q=${encodeURIComponent(address)}`} frameBorder="0" allowFullScreen="" className={overviewRtStyles.mapIframe} loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={tt("ui.google.map.7ac7", "Google Map")} />
        </div>}

      <div className={overviewRtStyles.rightBox}>
        <h3 className={`${overviewRtStyles.headerH3} ${tournamentStyles.headerH3}`}>{tt("ui.social.links.339c", "Social Links")}</h3>

        <div className={`${profileStyles.socialLinksListContainer} ${overviewRtStyles.socialLinksListContainer}`}>
          {Array.isArray(socialLinks) && socialLinks.length === 0 ? <div className={profileStyles.noInterestsContainer}>
              <h4 className={profileStyles.profileH4HeaderEmptyContent}>{tt("ui.no.social.links.yet.57b4", "No social links yet")}</h4>
              <p className={profileStyles.emptyParagraphContent}>{tt("ui.no.social.media.links.cfa8", "No social media links have been added for this event")}</p>
            </div> : <>
              {socialLinks.slice(0, showMoreSocials ? socialLinks.length : 4).map((socialLink, index) => <Link href={socialLink.url} key={index} target='_blank' rel='noopener noreferrer' className={`${profileStyles.interest1} ${overviewRtStyles.socialLinks} ${profileStyles.topMostLayerColor}`}>
