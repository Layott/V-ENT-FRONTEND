...
const EditUserProfileInterests = ({
  ...
  const filterInterests = query => {
    return allInterests.filter(interest => interest.toLowerCase().includes(query.toLowerCase()));
  };
  useEffect(() => {
    if (searchQuery.trim() !== "") {
  ...
  return <div className={styles.editInterestsContainer}>
            <h3>{tt("ui.profile.interests.1326", "Profile Interests")}</h3>
