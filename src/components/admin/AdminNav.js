...
const NAV = [
  {
    section: 'Overview',
    items: [
      { label: 'Dashboard', href: '/admin', icon: MdOutlineDashboard, roles: null, perms: null },
    ],
  },
  {
    section: 'Management',
    items: [
      ...
      { label: 'Users', href: '/admin/users', icon: LuUsers, roles: ['super', 'moderator', 'finance', 'support'], perms: ['view_users'], badge: 'users' },
      { label: 'Tournaments', href: '/admin/tournaments', icon: RiTrophyLine, roles: ['super', 'moderator'], perms: ['cancel_tournament', 'override_match_score', 'resolve_dispute'] },
      { label: 'Disputes', href: '/admin/disputes', icon: MdGavel, roles: ['super', 'moderator'], perms: ['resolve_dispute'], badge: 'disputes' },
      { label: 'KYC Review', href: '/admin/kyc', icon: LuShield, roles: ['super', 'moderator', 'finance', 'support'], perms: ['list_kyc', 'approve_kyc', 'reject_kyc'], badge: 'kyc' },
    ],
  },
  {
    section: 'Finance',
    items: [
      { label: 'Payouts', href: '/admin/payouts', icon: IoWalletOutline, roles: ['super', 'finance'], perms: ['list_payouts', 'approve_payouts'], badge: 'payouts' },
    ],
  },
  {
    section: 'System',
    items: [
      ...
      { label: 'Settings', href: '/admin/settings', icon: MdOutlineSettings, roles: ['super'], perms: ['manage_admins'] },
    ],
  },
]
...
export default function AdminNav({ admin, onLogout, badges = {}, sidebarOpen = false }) {
  ...
  return (
    <aside className={`${styles.sidebar} ${sidebarOpen ? styles.open : ''}`} id="admin-sidebar">
      {/* Logo */}
            ...
            return (
              <li key={section}>
                ...
                <ul className={styles.sectionList}>
                  {visibleItems.map(({ label, href, icon: Icon, badge: badgeKey }) => (
                    <li
                      key={href}
                      className={`${styles.navItem} ${isActive(href) ? styles.activeItem : ''}`}
                    >
                      <Link className={styles.navLink} href={href}>
                        <Icon className={styles.navIcon} />
                        {label}
                        {badgeKey && badges[badgeKey] > 0 && (
                          <span className={styles.navBadge}>{badges[badgeKey]}</span>
                        )}
                      </Link>
                    </li>
                  ))}
