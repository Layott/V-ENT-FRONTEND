...
const ICONS = {
  ...
  steam: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="#c7d5e0" aria-hidden="true">
...
const PROVIDERS = [
  { id: 'google', label: 'Google', sub: 'How this account signs in.', linkable: false },
  { id: 'discord', label: 'Discord', sub: 'Confirm your Discord handle on your profile.', linkable: true },
  { id: 'steam', label: 'Steam', sub: 'Confirm your Steam account on your profile.', linkable: true },
];
...
const LinkedAccountsPanel = ({ showToast }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError('Could not load your linked accounts.');
    } finally {
      setLoading(false);
    }
  }, [apiBase, token]);
  ...
  useEffect(() => {
    ...
    ['discord', 'steam'].forEach((id) => {
      ...
      if (!outcome) return;
      showToast?.(outcome === 'linked'
        ? `${id[0].toUpperCase()}${id.slice(1)} linked`
        : `${id[0].toUpperCase()}${id.slice(1)} linking did not complete`);
      params.delete(id);
    });
    ...
    <div className={shared.formStack}>
      <div className={shared.card}>
        ...
        <p className={shared.cardSub}>
          A linked account is confirmed by the platform itself, so the handle on your profile is
        ...
        {loading && <p className={shared.cardSub}>Loading your linked accounts...</p>}
                  ...
                  <div className={styles.meta}>
                    ...
                    <div className={styles.sub}>
                      {connected && state.label
                        ? state.label
                        : (!configured && p.linkable ? 'Not set up yet.' : p.sub)}
                    </div>
                  </div>

                  {p.id === 'google' ? (
                    <span className={styles.sub}>{connected ? 'Sign-in method' : 'Not used'}</span>
                  ) : (
                    <button
                      type="button"
