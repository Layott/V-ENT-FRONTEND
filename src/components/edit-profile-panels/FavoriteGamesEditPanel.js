...
const platformClass = (p) => {
  const map = { PC: styles.tagPc, PS5: styles.tagPs, Xbox: styles.tagXbox, Mobile: styles.tagMobile };
  return map[p] || '';
};
...
const FavoriteGamesEditPanel = ({ initialGames, onSave, onCancel, showToast }) => {
  const seedGames = (initialGames && initialGames.length > 0
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/games/`, { signal: controller.signal });
        if (!res.ok) return;
  ...
  const setAsMain = (id) => {
    setGames(games.map((g) => ({ ...g, isMain: g.id === id })));
  };
  ...
  const updateGamertag = (id, value) => {
    setGames(games.map((g) => g.id === id ? { ...g, gamertag: value } : g));
  };
  ...
  const addGameFromCatalog = (cg) => {
    ...
    setGames([...games, { ...cg, platforms: ['PC', 'PS5'], gamertag: '', gamertagLabel: 'Gamertag', isMain: games.length === 0 }]);
    setModalOpen(false);
    setModalSearch('');
  };
      ...
      <div className={shared.card}>
        <div className={styles.gamesHead}>
          <div className={styles.gamesHeadLeft}>
            <h3 className={styles.gamesTitle}>Favorite Games</h3>
            <span className={styles.gamesSub}>Set your main game and add the gamertag we should display next to each title.</span>
          </div>
        ...
        <div className={styles.gamesGrid}>
          {filtered.length === 0 ? (
            <div className={styles.gamesEmpty}>No games match your search.</div>
          ) : filtered.map((g) => (
            <article className={`${styles.gameCard} ${g.isMain ? styles.gameCardMain : ''}`} key={g.id}>
              <div className={styles.gameCoverWrap}>
                ...
                {g.cover ? <img src={g.cover} alt={g.name} /> : <span className={styles.coverFallback}>{(g.name || '?').charAt(0)}</span>}
              </div>
              <div className={styles.gameBody}>
                <h4 className={styles.gameTitle}>{g.name}</h4>
                <div className={styles.platformRow}>
                  {(g.platforms || []).map((p) => (
                    <span key={p} className={`${styles.platformTag} ${platformClass(p)}`}>{p}</span>
                  ))}
                </div>
                <div className={styles.gtagRow}>
                  ...
                  <input
                    type="text"
                    value={g.gamertag}
                    placeholder={g.gamertagLabel}
                    onChange={(e) => updateGamertag(g.id, e.target.value)}
                  />
                </div>
                {g.isMain ? (
                  <div className={styles.mainMarker}>
                    <StarSvg />
                    Main game
                  </div>
                ) : (
                  <button type="button" className={styles.setMainBtn} onClick={() => setAsMain(g.id)}>
                    <StarSvg />
                    Set as main
                  </button>
                )}
      ...
      {modalOpen && (
        <div className={styles.overlay} onClick={(e) => { if (e.target === e.currentTarget) setModalOpen(false); }}>
          <div className={styles.modal}>
            ...
            <h3 className={styles.modalTitle}>Add a game</h3>
            <p className={styles.modalSub}>Pick from the V-ENT catalog. Only games with active tournaments are listed.</p>
            <div className={styles.modalSearch}>
              ...
              <input type="text" placeholder="Search the catalog" value={modalSearch} onChange={(e) => setModalSearch(e.target.value)} />
            </div>
            <div className={styles.modalList}>
              {filteredCatalog.map((cg) => (
                <div className={styles.modalGame} key={cg.id} onClick={() => addGameFromCatalog(cg)} role="button" tabIndex={0}>
                  <div className={styles.modalGameCover}>{cg.cover ? <img src={cg.cover} alt="" /> : <span className={styles.coverFallback}>{(cg.name || '?').charAt(0)}</span>}</div>
                  <span className={styles.modalGameName}>{cg.name}</span>
                </div>
              ))}
            </div>
            <div className={styles.modalFoot}>
              <button type="button" className={`${shared.btn} ${shared.ghostBTN}`} onClick={() => setModalOpen(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
