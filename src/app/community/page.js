...
import { FaHeart, FaRegHeart, FaRegComment, FaShare, FaBookmark, FaRegBookmark, FaComments, FaUsers, FaEnvelope, FaCrosshairs, FaImage, FaPaperPlane, FaPoll, FaThumbtack, FaLock, FaSearch, FaPlus, FaCheck } from 'react-icons/fa';
...
const CommunityInner = () => {
  ...
  // Reading the community needs no account; writing to it does. Derived
  // from the token rather than from `status`, because a resolved session
  // with no token cannot write either and would fail the same way.
  ...
  const handleTabChange = id => {
    ...
    if (id === 'feed') params.delete('tab');else params.set('tab', id);
  ...
  const handleJoinClub = async id => {
    ...
    try {
      await fetch(`${apiUrl}/club/${id}/join/`, {
        method: 'POST',
        headers: authHeaders()
      });
    } catch (err) {
  ...
  const openDm = async convo => {
    ...
    try {
      const res = await fetch(`${apiUrl}/dm/${convo.id}/`, {
        headers: authHeaders()
      });
      const data = await res.json();
      if (data.status === 'success') {
        ...
        setDmThreads(prev => prev.map(t => t.id === convo.id ? {
          ...t,
          unread_count: 0
        } : t));
      } else {
  ...
  const handleSendDm = async () => {
    ...
    const replaceLocal = (messages, serverMsg) => serverMsg ? (messages || []).map(m => m.id === localId ? serverMsg : m) : (messages || []).filter(m => m.id !== localId);
    try {
  ...
  const handleStartDm = async () => {
    ...
    try {
      const res = await fetch(`${apiUrl}/dm/new/send/`, {
        ...
        headers: authHeaders(),
        body: JSON.stringify({
          username: newDmUsername.trim().replace(/^@/, ''),
          body: newDmBody.trim()
        })
      });
      const data = await res.json();
  ...
  const handleAcceptScrim = scrim => {
    ...
    if (eligible.length === 0) {
      setScrimError(myTeams.length === 0 ? 'You need a team before you can accept a scrim.' : `${scrim.team_a?.name} is your own team. Bring a different one to accept.`);
      return;
    }
      ...
      <main className={styles.mainContainer}>
        ...
        <div className={styles.rightPaneContainer}>
          ...
          <div className={styles.tabs} role="tablist">
            {TABS.map(t => <button key={t.id} className={`${styles.tabBtn} ${activeTab === t.id ? styles.tabActive : ''}`} onClick={() => handleTabChange(t.id)} role="tab" aria-selected={activeTab === t.id}>
          ...
          {/* ─── FEED ─── */}
          {activeTab === 'feed' && <div className={styles.feedLayout}>
              <div className={styles.feedSearchBar}>
                <FaSearch className={styles.feedSearchIcon} />
                <input className={styles.feedSearchInput} placeholder={tt("ui.search.posts.users.f1e6", "Search posts or users...")} value={feedQuery} onChange={e => setFeedQuery(e.target.value)} />
              </div>

              {!signedIn ? <SignInToEngage action={tt("community.toPost", "to post to the feed.")} /> : <div className={styles.composeBox}>
                ...
                <div className={styles.composeBody}>
                  ...
                  <div className={styles.composeActions}>
                    <button type="button" className={styles.composeImageBtn} onClick={handleAttachImage}>
                      <FaImage /> {composeImage ? tx("Replace image") : tx("Attach image")}
                    </button>
              ...
              {feedLoading ? <p className={styles.stateText}>{tt("ui.loading.feed.5ee0", "Loading feed...")}</p> : filteredPosts.length === 0 ? <p className={styles.stateText}>
                  {feedQuery ? tx("No posts match your search.") : tx("No posts yet. Be the first.")}
                    ...
                    {post.images && post.images.length > 0 && <Link href={`/community/post/${post.slug || post.id}`} className={styles.postBodyLink}>
                        {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          {/* ─── FORUMS ─── */}
          {activeTab === 'forums' && <div className={styles.forumsLayout}>
              <div className={styles.categoryList} role="tablist" aria-label={tt("ui.forum.categories.d09f", "Forum categories")}>
                {FORUM_CATEGORIES.map(cat => <button key={cat} className={`${styles.categoryBtn} ${forumCategory === cat ? styles.categoryActive : ''}`} onClick={() => setForumCategory(cat)}>
                    <span>{cat}</span>
                  </button>)}
                {signedIn && <button className={`${styles.newThreadCta} goldBTN`} onClick={() => setShowNewThread(s => !s)}>
                  <FaPlus /> {tt("ui.new.thread.0205", "New thread")}
                </button>}
              </div>

              <div className={styles.threadColumn}>
                {showNewThread && <div className={styles.newThreadBox}>
                    <h2 className={styles.newThreadTitle}>{tt("ui.start.new.thread.87d3", "Start a new thread")}</h2>
                    <input className={styles.newThreadInput} placeholder={tt("ui.thread.title.1cde", "Thread title")} value={newThreadTitle} onChange={e => setNewThreadTitle(e.target.value)} maxLength={120} />
                    <select className={styles.newThreadSelect} value={newThreadCategory} onChange={e => setNewThreadCategory(e.target.value)}>
                      {FORUM_CATEGORIES.filter(c => c !== 'All').map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                    <textarea className={styles.newThreadTextarea} placeholder={tt("ui.write.post.1519", "Write your post...")} value={newThreadBody} onChange={e => setNewThreadBody(e.target.value)} maxLength={2000} />
                    <div className={styles.newThreadActions}>
                      <button className={styles.newThreadCancel} onClick={() => setShowNewThread(false)}>
                        {tt("ui.cancel.77df", "Cancel")}
                      </button>
                      <button className={`${styles.newThreadSubmit} goldBTN`} onClick={handleCreateThread} disabled={!newThreadTitle.trim() || !newThreadBody.trim()}>
                        {tt("ui.post.thread.d1cf", "Post thread")}
                      </button>
                    </div>
                  </div>}
                  ...
                  {threadsLoading ? <p className={styles.stateText}>{tt("ui.loading.threads.4648", "Loading threads...")}</p> : filteredThreads.length === 0 ? <p className={styles.stateText}>{tt("ui.no.threads.category.0175", "No threads in this category.")}</p> : filteredThreads.map(thread => <Link key={thread.id} href={`/community/thread/${thread.slug || thread.id}`} className={styles.threadRow}>
                        ...
                        <div className={styles.threadMain}>
                          <h2 className={styles.threadTitle}>
                            {thread.is_pinned && <FaThumbtack className={styles.threadPin} title={tt("ui.pinned.f931", "Pinned")} />}
                            {thread.is_locked && <FaLock className={styles.threadLock} title={tt("ui.locked.a798", "Locked")} />}
                            {tx(thread.title)}
                          </h2>
                          <div className={styles.threadMeta}>
                            <span className={styles.threadCategoryPill}>{thread.category}</span>
                            <span>by @{thread.author.username}</span>
                            <span>{tt("ui.updated.f2f8", "Updated")} {relativeTime(thread.last_activity)}</span>
                          </div>
                        </div>
                        <div className={styles.threadStats}>
                          <span className={styles.threadReplyCount}>{thread.reply_count}</span>
                          <span className={styles.threadReplyLabel}>{tt("ui.replies.6f56", "replies")}</span>
                        </div>
                      </Link>)}
                </div>
              </div>
              ...
              <div className={styles.clubsGrid}>
                {clubsLoading ? <p className={styles.stateText}>{tt("ui.loading.clubs.0f67", "Loading clubs...")}</p> : filteredClubs.length === 0 ? <p className={styles.stateText}>{tt("ui.no.clubs.match.search.31f3", "No clubs match your search.")}</p> : filteredClubs.map(club => <div key={club.id} className={styles.clubCard}>
                      ...
                      <div className={styles.clubFoot}>
                        {club.is_owner ? <button className={styles.clubJoinedBtn} disabled>
                            <FaCheck /> {tt("ui.owner.89ff", "Owner")}
                          </button> : club.is_joined ? <button className={styles.clubJoinedBtn} disabled>
                            <FaCheck /> {tt("ui.joined.43a1", "Joined")}
                          </button> : <button className={`${styles.clubJoinBtn} goldBTN`} onClick={() => handleJoinClub(club.id)} disabled={!signedIn} title={signedIn ? undefined : tt("community.signInToJoin", "Sign in to join this club")}>
                            {tt("ui.join.e0d7", "Join")}
                          </button>}
                      </div>
                    </div>)}
              </div>
          ...
          {/* ─── DMs ─── */}
          {activeTab === 'dms' && <div className={styles.dmLayout}>
              <div className={styles.dmConvoListWrap}>
                <div className={styles.dmContactSearch}>
                  <FaSearch className={styles.feedSearchIcon} />
                  <input className={styles.feedSearchInput} placeholder={tt("ui.search.contacts.6435", "Search contacts...")} value={dmContactQuery} onChange={e => setDmContactQuery(e.target.value)} />
                </div>

                <button className={`${styles.dmNewBtn} goldBTN`} onClick={() => setNewDmOpen(v => !v)}>
                  <FaPlus /> {tt("ui.new.message.1ed2", "New message")}
                </button>
                  ...
                  if (e.key === 'Enter') openDm(convo);
                          ...
                          <div className={styles.dmConvoMain}>
                            ...
                            <span className={styles.dmConvoPreview}>
                              {convo.last_message || tx("No messages yet")}
                            </span>
                          </div>
                          {convo.unread_count > 0 && <span className={styles.dmUnreadBadge}>{convo.unread_count}</span>}
                        </div>;
              })}
                </div>
              </div>
              ...
              <div className={styles.dmThreadPane}>
                {activeDm ? <>
                    <div className={styles.dmThreadHeader}>
                      <div className={styles.dmConvoAvatar}>
                        <Avatar src={otherOf(activeDm)?.avatar} name={otherOf(activeDm)?.full_name || otherOf(activeDm)?.username || 'user'} size={38} />
                      </div>
                      <div>
                      ...
                      <Link href={`/community/dm/${activeDm.slug || activeDm.id}`} className={styles.dmFullScreenLink}>
                        {tt("ui.full.screen.d15f", "Full screen")}
                      </Link>
                    </div>
                    <div className={styles.dmThreadMessages}>
                      {(activeDm.messages || []).length === 0 ? <p className={styles.stateText}>{tt("ui.no.messages.yet.send.8d65", "No messages yet. Send the first one.")}</p> : activeDm.messages.map(msg => {
                  const isMine = Boolean(msg.mine);
                  return <div key={msg.id} className={`${styles.dmBubble} ${isMine ? styles.dmBubbleSent : styles.dmBubbleReceived}`}>
                              {msg.body}
                            </div>;
                })}
                    </div>
                    ...
                    <div className={styles.dmCompose}>
                      <input className={styles.dmComposeInput} placeholder={tt("ui.type.message.09bd", "Type a message...")} value={dmInput} onChange={e => {
                  setDmInput(e.target.value);
                  if (dmError) setDmError('');
                }} onKeyDown={e => {
                  if (e.key === 'Enter') handleSendDm();
                }} />
                      <button className={`${styles.dmSendBtn} redBTN`} onClick={handleSendDm} disabled={!dmInput.trim() || dmSending}>
                        <FaPaperPlane />
                      </button>
                    </div>
                  </> : <div className={styles.dmEmpty}>
                    <FaEnvelope className={styles.dmEmptyIcon} />
                    <p>{tt("ui.pick.conversation.d940", "Pick a conversation")}</p>
                  </div>}
              </div>
            </div>}
          ...
          {activeTab === 'scrims' && <div className={styles.scrimsSection}>
              <div className={styles.scrimsHeaderRow}>
                ...
                {signedIn && <Link href="/community/scrim/create" className={`${styles.scrimChallengeCta} goldBTN`}>
                  <FaPlus /> {tt("ui.challenge.bb9d", "Challenge")}
                </Link>}
              </div>
              ...
              return <div key={scrim.id} className={styles.scrimRow}>
                        ...
                        <div className={styles.scrimCol} data-label={tt("ui.opponent.4548", "Opponent")}>
                          {opponent ? <>
                              ...
                              {opponent.name}
                            </> : <span className={styles.scrimOpenSlot}>{tt("ui.open.slot.2189", "Open slot")}</span>}
                        </div>
