...
const ThreadInner = ({ slug: slugFromPath }) => {
  ...
  useEffect(() => {
    ...
    const fetchThread = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${apiUrl}/thread/${id}/`, { headers: authHeaders });
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      if (data.status === 'success' && data.data?.reply) {
        ...
        setReplyError(data.message || 'Could not post that reply.');
      }
    } catch (err) {
  ...
  const toggleThreadUpvote = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.status === 'success') {
        setThread((t) => (t ? { ...t, upvotes: data.data.upvotes, upvoted: data.data.upvoted } : t));
      }
    } catch (err) {
  ...
  const toggleReplyUpvote = async (replyId) => {
    ...
    try {
      ...
      if (data.status === 'success') {
        setReplies((rows) =>
          rows.map((r) =>
            r.id === replyId ? { ...r, upvotes: data.data.upvotes, upvoted: data.data.upvoted } : r,
          ),
        );
      }
    } catch (err) {
  ...
  const pagedReplies = useMemo(
    () => replies.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE),
    [replies, page],
  );
        ...
        <div className={styles.rightPaneContainer}>
          <div className={styles.threadShell}>
            <button
              ...
              onClick={() => router.push('/community?tab=forums')}
            >
              <FiArrowLeft /> Back to forums
            </button>

            {loading ? (
              <p className={styles.stateText}>Loading thread...</p>
            ) : !thread ? (
              <p className={styles.stateText}>Thread not found.</p>
            ) : (
              <>
                <article className={styles.parentPost}>
                  <div className={styles.postHead}>
                    ...
                    <div className={styles.authorInfo}>
                      <span className={styles.authorName}>{thread.author.full_name}</span>
                  ...
                  <div className={styles.threadFlags}>
                    ...
                    {thread.is_locked && (
                      <span className={`${styles.flag} ${styles.flagLocked}`}>
                        <FaLock /> Locked
                      </span>
                    )}
                  </div>
                  ...
                  <div className={styles.threadStatsRow}>
                    <button
                      type="button"
                      className={`${styles.upvoteBtn} ${thread.upvoted ? styles.upvoteBtnOn : ''}`}
                      onClick={toggleThreadUpvote}
                      disabled={!session?.user?.sessionToken}
                      title={session?.user?.sessionToken ? 'Upvote this thread' : 'Log in to upvote'}
                    >
                ...
                <div className={styles.replyHeaderRow}>
                  <p className={styles.replyCount}>
                    ...
                    {totalPages > 1 && <span className={styles.pageNote}> · Page {page} of {totalPages}</span>}
                  </p>
                </div>

                <div className={styles.replyList}>
                  {pagedReplies.length === 0 ? (
                    <p className={styles.stateText}>No replies yet. Be the first.</p>
                  ) : (
                      ...
                      <div key={reply.id} className={styles.replyCard}>
                        ...
                        <div className={styles.replyBody}>
                          <div className={styles.replyHeader}>
                            <span className={styles.replyAuthor}>{reply.author.full_name}</span>
                          ...
                          <div className={styles.replyFooter}>
                            <button
                              type="button"
                              className={`${styles.upvoteBtn} ${reply.upvoted ? styles.upvoteBtnOn : ''}`}
                              onClick={() => toggleReplyUpvote(reply.id)}
                              disabled={!session?.user?.sessionToken}
                              title={session?.user?.sessionToken ? 'Upvote this reply' : 'Log in to upvote'}
                            >
                  ...
                  <div className={styles.pagination}>
                    ...
                    <div className={styles.pageNumbers}>
                      {Array.from({ length: totalPages }).map((_, i) => (
                        <button
                          key={i}
                          className={`${styles.pageNumberBtn} ${page === i + 1 ? styles.pageNumberActive : ''}`}
                          onClick={() => setPage(i + 1)}
                        >
                          {i + 1}
                        </button>
                ...
                {thread.is_locked ? (
                  <div className={styles.lockedNotice}>
                    <FaLock /> This thread is locked. New replies are disabled.
                  </div>
                ) : (
                  <div className={styles.composeBox}>
                    ...
                    <div className={styles.composeBody}>
                      <div className={styles.markdownToolbar}>
                        <button type="button" className={styles.mdBtn} onClick={() => insertMarkdown('**', '**', 'bold')} title="Bold">
                          <FaBold />
                        </button>
                        <button type="button" className={styles.mdBtn} onClick={() => insertMarkdown('*', '*', 'italic')} title="Italic">
                          <FaItalic />
                        </button>
                        <button type="button" className={styles.mdBtn} onClick={() => insertMarkdown('`', '`', 'code')} title="Code">
                          <FaCode />
                        </button>
                        <button type="button" className={styles.mdBtn} onClick={() => insertMarkdown('> ', '', 'quote')} title="Quote">
                          <FaQuoteRight />
                        </button>
                        <button type="button" className={styles.mdBtn} onClick={() => insertMarkdown('- ', '', 'item')} title="List">
                          <FaListUl />
                        </button>
                        <button type="button" className={styles.mdBtn} onClick={() => insertMarkdown('[', '](https://)', 'link text')} title="Link">
                          <FaLink />
                        </button>
                        <span className={styles.mdHint}>Markdown supported</span>
                      </div>
                      <textarea
                        ref={textareaRef}
                        className={styles.composeTextarea}
                        placeholder="Write a reply... Use markdown for formatting."
                        value={replyText}
                      ...
                      <div className={styles.composeActions}>
                        ...
                        <button
                          className={`${styles.replyBtn} goldBTN`}
                          onClick={handleReply}
