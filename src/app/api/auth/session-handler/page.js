...
export default function SessionHandler() {
  ...
  return <div className="flex flex-col items-center justify-center min-h-screen">
      <div className="p-8 bg-white rounded shadow-md max-w-md w-full">
        <h1 className="text-2xl font-bold mb-4 text-center">{tt("ui.authentication.ee1a", "Authentication")}</h1>
        <div className="text-center">
          <p className="mb-2">{status}</p>
          {error && <p className="text-red-500 mb-4">{error}</p>}
          {error && <button onClick={() => router.push('/login')} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
              {tt("ui.return.login.3eb8", "Return to Login")}
            </button>}
        </div>
      </div>
    </div>;
