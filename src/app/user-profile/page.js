            ...
            <div className={styles.heroMeta}>
              ...
              <div className={styles.heroActions}>
                ...
                {isOwner ? <Link href="/edit-user-profile" className={styles.heroBtn}>
                    ...
                    {/* When there is no Message button, SAY why. Without this a
                        viewer cannot tell "they have messages off" from "the
                        page is broken", and the second is what people assume.
                        It reports the person's OWN stated setting, never
                        anything derived from the viewer: somebody who has been
                        blocked sees exactly what somebody who simply is not
                        allowed sees. */}
                    {!isOwner && !canMessage && profileData?.message_policy === 'nobody' && <span className={styles.heroNote}>
                      {tt('profile.dmNobody', 'Not accepting messages')}
                    </span>}
                    {!isOwner && !canMessage && profileData?.message_policy === 'followers' && <span className={styles.heroNote}>
                      {tt('profile.dmFollowers', 'Only accepts messages from people they follow')}
                    </span>}
