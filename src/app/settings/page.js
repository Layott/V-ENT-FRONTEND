...
const SettingsContent = () => {
  ...
  const setPanel = (id) => {
    router.push(`/settings?panel=${id}`, { scroll: false });
  };

  const showToast = useCallback((msg, kind = 'ok') => {
    setToast({ msg, kind });
    setTimeout(() => setToast({ msg: '', kind: 'ok' }), 2500);
  }, []);
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      if (token) {
        try {
          const [setRes, devRes] = await Promise.all([
            fetch(`${apiBase}/setting/`, { headers: authHeaders() }),
            fetch(`${apiBase}/device/list/`, { headers: authHeaders() }),
          ]);
      ...
      try {
        ...
        if (userId) {
          const res = await fetch(
            `${apiBase}/auth/get-user-informations/?user_id=${userId}`,
            { headers: authHeaders() }
          );
  ...
  const handleSaveNotifications = async (next) => {
    ...
    if (out?.status === 'success') {
      setSettings((prev) => ({ ...(prev || {}), notifications: out.data?.notifications || next }));
  ...
  const handleSavePrivacy = async (next) => {
    ...
    if (out?.status === 'success') {
      setSettings((prev) => ({ ...(prev || {}), privacy: out.data?.privacy || next }));
  ...
  const handleSaveSecurity = async (next) => {
    ...
    if (out?.status === 'success') {
      setSettings((prev) => ({ ...(prev || {}), security: out.data?.security || next }));
  ...
  const handleSavePayments = async (next) => {
    ...
    if (out?.status === 'success') {
      setSettings((prev) => ({ ...(prev || {}), payments: out.data?.payments || next }));
      showToast('Payment preferences saved');
    } else {
      showToast(out?.message || 'Save failed', 'error');
    }
  };
  ...
  const handleRevokeDevice = async (deviceId) => {
    ...
    if (out?.status === 'success') {
      ...
      showToast(out?.message || 'Could not sign out device', 'error');
    }
  };
  ...
  const handleRevokeAllOthers = async () => {
    ...
    try {
      const res = await fetch(`${apiBase}/device/list/`, { headers: authHeaders() });
      const data = await res.json();
        ...
        <main className={styles.mainContainer}>
          ...
          <div className={styles.rightPaneContainer}>
            <div className={styles.pageHeader}>
              ...
              <p className={styles.pageSub}>Manage your account, security, and preferences.</p>
            </div>
              ...
              <aside className={shared.submenuCard}>
                ...
                <ul className={shared.submenuList}>
                  {PANELS.map((p) => {
                    ...
                    const cls = [
                      shared.submenuItem,
                      active ? shared.submenuItemActive : '',
                      p.danger ? shared.submenuItemDanger : '',
                    ].filter(Boolean).join(' ');
                    return (
      ...
      <main className={styles.mainContainer}>
        ...
        <div className={styles.rightPaneContainer}>
          ...
          <div className={shared.pageGrid}>
            ...
            <section>
              {activePanel === 'account' && (
                <AccountPanel user={user} onSave={handleSaveAccount} showToast={showToast} />
              )}
              ...
              {activePanel === 'linked' && (
                <LinkedAccountsPanel showToast={showToast} />
              )}
              {activePanel === 'danger' && (
                <DangerZonePanel showToast={showToast} />
              )}
            </section>
          </div>
        </div>
