...
const RankingsView = () => {
  ...
  const fetchRankings = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/ranking/?${params.toString()}`, {
        headers
      });
  ...
  const renderTrend = entry => {
    ...
    if (delta === 0) {
      return <span className={`${styles.trend} ${styles.trendFlat}`}>
          <LuMinus />
          <span>0</span>
        </span>;
    }
    if (delta > 0) {
      return <span className={`${styles.trend} ${styles.trendUp}`}>
          <LuTrendingUp />
          <span>+{delta}</span>
        </span>;
    }
    return <span className={`${styles.trend} ${styles.trendDown}`}>
        <LuTrendingDown />
        <span>{delta}</span>
      </span>;
  };
  const podiumIcons = [<LuCrown key="1" className={styles.podiumIcon} />, <LuMedal key="2" className={styles.podiumIcon} />, <LuMedal key="3" className={styles.podiumIcon} />];
  const podiumLabels = ['1st', '2nd', '3rd'];
  const podiumOrderClass = [styles.podiumGold, styles.podiumSilver, styles.podiumBronze];
  const positionsToTop100 = sessionUserEntry ? Math.max(0, sessionUserEntry.rank - 100) === 0 ? sessionUserEntry.rank <= 100 ? null : sessionUserEntry.rank - 100 : sessionUserEntry.rank - 100 : null;
  return <div className={styles.rankingsContainer}>
      {/* Header */}
      <div className={styles.headerRow}>
        <div>
          <h1 className={styles.pageTitle}>{tt("ui.rankings.f5fe", "Rankings")}</h1>
          <p className={styles.pageSub}>{tt("ui.top.players.teams.organizations.399d", "Top players, teams and organizations across V-ENT.")}</p>
        </div>
      </div>

      {/* Tabs */}
      <div className={styles.tabsRow}>
        {TABS.map(t => <button key={t.id} type="button" className={`${styles.tabBTN} ${tab === t.id ? styles.activeTab : ''}`} onClick={() => {
        setTab(t.id);
        setExpandedRow(null);
      }}>
            {tx(t.label)}
      ...
      <div className={styles.filtersRow}>
        ...
        <div className={styles.filterSelect}>
          <select value={sortBy} onChange={e => setSortBy(e.target.value)} className={styles.select}>
            {SORTS.map(s => <option key={s.value} value={s.value}>{tt("ui.sort.8b33", "Sort:")} {tx(s.label)}</option>)}
          </select>
          <TiArrowSortedDown className={styles.selectCaret} />
        </div>
      </div>
      ...
      {/* Podium */}
      {!loading && top3.length === 3 && <div className={styles.podiumWrap}>
          {[1, 0, 2].map(slot => {
        ...
        return <button type="button" key={entry.id} className={`${styles.podiumCard} ${podiumOrderClass[slot]}`} onClick={() => navigateToProfile(entry)} aria-label={`View ${entry.name}`}>
                ...
                <div className={styles.podiumStats}>
                  ...
                  <div className={styles.podiumStat}>
                    ...
                    <span className={styles.podiumStatLabel}>{tt("ui.win.rate.79bc", "Win rate")}</span>
                  </div>
                </div>
      ...
      <div className={styles.tableWrap}>
        <div className={styles.tableHeaderRow}>
          <div className={`${styles.col} ${styles.colRank}`}>{tt("ui.rank.dd48", "Rank")}</div>
          <div className={`${styles.col} ${styles.colName}`}>{tab === 'players' ? 'Player' : tab === 'teams' ? 'Team' : 'Organization'}</div>
          <div className={`${styles.col} ${styles.colRegion}`}>{tt("ui.region.0f21", "Region")}</div>
          <div className={`${styles.col} ${styles.colPoints}`}>{tt("ui.points.4b2a", "Points")}</div>
          <div className={`${styles.col} ${styles.colWl}`}>W-L</div>
          <div className={`${styles.col} ${styles.colWinRate}`}>{tt("ui.win.rate.79bc", "Win rate")}</div>
          <div className={`${styles.col} ${styles.colTrend}`}>{tt("ui.trend.dae0", "Trend")}</div>
          <div className={`${styles.col} ${styles.colActions}`}>{tt("ui.actions.c3cd", "Actions")}</div>
        </div>
        ...
        {!loading && !error && sortedList.length === 0 && <div className={styles.emptyState}>
            <p className={styles.emptyTitle}>{tt("ui.no.results.b993", "No results")}</p>
            <p className={styles.emptySub}>{tt("ui.try.adjusting.filters.search.289d", "Try adjusting your filters or search term.")}</p>
          </div>}

        {!loading && !error && sortedList.length > 0 && <div className={styles.tableBody}>
            {sortedList.map(entry => {
          const isMe = entry.is_session_user;
          const isExpanded = expandedRow === entry.id;
          return <div key={entry.id} className={`${styles.row} ${isMe ? styles.rowMe : ''}`} role="button" tabIndex={0} onClick={() => navigateToProfile(entry)} onKeyDown={e => {
            if (e.key === 'Enter') navigateToProfile(entry);
          }}>
                  ...
                  <div className={`${styles.col} ${styles.colName}`}>
                    <div className={styles.nameCell}>
                      <div className={styles.avatarWrap}>
                        <RankAvatar src={entry.avatar} name={entry.name} size={36} className={styles.avatar} />
                      </div>
                  ...
                  <div className={`${styles.col} ${styles.colWinRate}`}>
                    ...
                    width: `${Math.min(100, entry.win_rate)}%`
                  }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className={`${styles.col} ${styles.colActions}`}>
                    <button type="button" className={`${styles.actionBtn} ${styles.actionGhost}`} onClick={e => {
                e.stopPropagation();
                navigateToProfile(entry);
              }}>
                      {tt("ui.view.69bd", "View")}
                    </button>
                    <button type="button" className={`${styles.actionBtn} ${following[entry.id] ? styles.actionFollowing : styles.actionFollow}`} onClick={e => toggleFollow(e, entry)}>
                      {following[entry.id] ? 'Following' : 'Follow'}
                    </button>
                  </div>

                  {/* Mobile expand toggle */}
                  <button type="button" className={styles.mobileExpandBtn} onClick={e => {
              e.stopPropagation();
              setExpandedRow(isExpanded ? null : entry.id);
            }} aria-label={tt("ui.toggle.details.5a92", "Toggle details")}>
                  ...
                  <div className={`${styles.mobileDetail} ${isExpanded ? styles.mobileDetailOpen : ''}`}>
                    <div className={styles.mobileDetailGrid}>
                      ...
                      <div className={styles.mobileDetailItem}>
                        <span className={styles.mobileDetailLabel}>{tt("ui.win.rate.79bc", "Win rate")}</span>
                        <span className={styles.mobileDetailValue}>{entry.win_rate}%</span>
                      </div>
                      <div className={styles.mobileDetailItem}>
                        <span className={styles.mobileDetailLabel}>{tt("ui.trend.dae0", "Trend")}</span>
                  ...
                  navigateToProfile(entry);
                        ...
                        {tt("ui.view.profile.b987", "View profile")}
                      </button>
                      <button type="button" className={`${styles.actionBtn} ${following[entry.id] ? styles.actionFollowing : styles.actionFollow}`} onClick={e => toggleFollow(e, entry)}>
                        {following[entry.id] ? 'Following' : 'Follow'}
                      </button>
      ...
      {tab === 'players' && sessionUserEntry && <div className={styles.yourRankStrip}>
          <div className={styles.yourRankInner}>
            <div className={styles.yourRankLeft}>
              <div className={styles.yourRankAvatarWrap}>
                <RankAvatar src={sessionUserEntry.avatar} name={sessionUserEntry.name} size={42} className={styles.yourRankAvatar} />
              </div>
              <div className={styles.yourRankInfo}>
                <span className={styles.yourRankLabel}>{tt("ui.rank.f655", "Your rank")}</span>
            ...
            <div className={styles.yourRankCenter}>
              <div className={styles.yourRankStat}>
                ...
                <span className={styles.yourRankStatLabel}>{tt("ui.rank.dd48", "Rank")}</span>
              </div>
              ...
              <div className={styles.yourRankStat}>
                ...
                <span className={styles.yourRankStatLabel}>{tt("ui.win.rate.79bc", "Win rate")}</span>
              </div>
              ...
              <div className={`${styles.yourRankStat} ${styles.yourRankTrendStat}`}>
                ...
                <span className={styles.yourRankStatLabel}>{tt("ui.trend.dae0", "Trend")}</span>
              </div>
            </div>
            <div className={styles.yourRankRight}>
              {sessionUserEntry.rank <= 100 ? <span className={styles.yourRankBadge}>
                  <HiOutlineTrophy /> {tt("ui.top.adaf", "Top 100")}
                </span> : <span className={styles.yourRankGap}>
                  {sessionUserEntry.rank - 100} {tt("ui.positions.top.3ecf", "positions to Top 100")}
                </span>}
