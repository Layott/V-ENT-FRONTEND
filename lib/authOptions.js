    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // is_staff decides where somebody lands after signing in. It used
          // to come only from the password provider's own login response, so a
          // sign-in finished through this provider - which is now every
          // sign-in that passes the second factor - sent an admin to /home.
          ...
          return {
            ...
            session_token: token,
            is_staff: isStaff,
            redirectUrl: isStaff ? "/admin" : "/home",
          };
        } catch {
          return null;
        }
...
async authorize(credentials, req) {
  try {
    ...
    // Whether this account can reach the admin console. It is not a permission
    // - the console keeps its own token and its own 2FA - it only decides
    // where somebody lands after signing in, so that a staff member does not
    // have to know and type /admin.
    const isStaff = Boolean(
      data.is_staff || (data.data && data.data.is_staff),
    );
  ...
  pages: {
    signIn: '/login',
    // The sign-out page. Without this, /api/auth/signout serves NextAuth's own
    // default: a bare card with a bright blue button, no V-ENT anywhere, and
    // English only, on a dark branded site. It is reachable by typing the
    // address and by any flow that redirects there, so leaving it as the
    // default means somebody eventually meets it.
    signOut: '/logout',
    error: '/login',
  },
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      if (token.session_token) {
        session.user = {
          ...
          isStaff: Boolean(token.is_staff),
    ...
    async jwt({ token, user, account }) {
      ...
      if (user?.session_token) {
        ...
        token.is_staff = Boolean(user.is_staff);

        // Staff land on the admin console, everybody else on the dashboard.
        token.redirectUrl = user.is_staff ? "/admin" : "/home";
      }
    ...
    async signIn({ user, account, profile }) {
      ...
      // A session token that authorize() already spent against the backend.
      // The backend only answers for a token it issued itself, so by the time
      // this runs the identity is established; there is nothing left to check.
      //
      // This was falling through to `return false` at the bottom, which reads
      // as AccessDenied. It went unnoticed while the only caller was the AFC
      // partner sign-in, and surfaced the moment the ordinary login started
      // using it to finish a sign-in that had passed the second factor.
      if (account.provider === 'external-token') {
        return true;
      }
