...
async authorize(credentials) {
  try {
    ...
    console.log("Backend response for credentials:", data);
    ...
    if (sessionToken) {
      ...
      console.log("Authorized user with credentials:", user);
      return user;
    } else {
    ...
    async signIn({ user, account, profile }) {
      ...
      if (account.provider === 'google' || account.provider === 'facebook') {
        try {
          ...
          // Add enhanced logging to track the token
          console.log("📝 Set user session token:", user.session_token);
