...
const ManageOrgContent = () => {
  ...
  const orgId = searchParams.get('id') || 'org_0';
  ...
  const linkTeam = (team) => {
    ...
    showToast(`${team.name} linked.`);
  };

  const unlinkTeam = (team) => {
    setTeams((prev) => prev.filter((t) => t.id !== team.id));
    showToast(`${team.name} unlinked.`);
  };
                      ...
                      {teams.map((t) => (
                        <li key={t.id} className={styles.teamRow}>
                          ...
                          <div className={styles.teamMeta}>
                            ...
                            <span className={styles.teamSub}>{t.game} · {t.member_count} members</span>
                          </div>
                          <button
                            type="button"
                            className={`${styles.miniBtn} ${styles.miniBtnDanger}`}
