...
const VendorShopContent = () => {
    ...
    const fetchVendors = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/event/${eventId}/vendors/`,
          { headers: authHeaders() }
        );
        const data = await res.json();
        if (data.status === 'success') {
  ...
  useEffect(() => {
    ...
    router.replace(`/events/vendor-shop?${params.toString()}`, { scroll: false });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [eventId]);
  ...
  const cartTotal = cart.reduce((sum, i) => sum + i.qty * i.price_ngn, 0);
        ...
        <div className={styles.rightPaneContainer}>
          ...
          <div className={styles.headerRow}>
            <div>
              <h2 className={styles.pageTitle}>
                <FaStore className={styles.titleIcon} /> Vendor Shop
              </h2>
              <p className={styles.pageSub}>
                Temporary marketplace for the event. On-site pickup or VIP delivery.
              </p>
            </div>
          ...
          {featuredProducts.length > 0 && (
            <div className={styles.featuredSection}>
              <h3 className={styles.sectionTitle}>
                <FaFire className={styles.fireIcon} /> Featured drops
              </h3>
                  ...
                  >
                    <div className={styles.featuredImgWrap}>
                      {p.image ? (
                        ...
                        <div className={styles.featuredImgFallback}><FaStore /></div>
                      )}
          ...
          <div className={styles.gridSection}>
            <div className={styles.gridHeader}>
              <h3 className={styles.sectionTitle}>Vendors at this event</h3>
              <span className={styles.resultCount}>
                {loading ? 'Loading…' : `${filteredVendors.length} vendors`}
              </span>
            </div>

            {loading ? (
              <p className={styles.stateText}>Loading vendors…</p>
            ) : filteredVendors.length === 0 ? (
              <div className={styles.emptyState}>
                <FaStore className={styles.emptyIcon} />
                <p className={styles.emptyTitle}>No vendors match.</p>
                <p className={styles.emptySub}>Try a different category or clear the search.</p>
              </div>
            ) : (
              <div className={styles.vendorGrid}>
                {filteredVendors.map((v) => (
                  <Link
                    key={v.id}
                    href={`/events/vendor-shop/vendor?event=${eventId}&vendor=${v.id}`}
                  ...
                  >
                    ...
                    <div className={styles.vendorBody}>
                      <div className={styles.vendorHead}>
                        ...
                        <div>
                          ...
                          <p className={styles.vendorMeta}>
                            {v.category} • Booth {v.booth || v.booth_number}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className={styles.vendorFoot}>
                        ...
                        <span className={styles.viewLink}>View stall →</span>
                      </div>
                    </div>
                  </Link>
                ))}
          ...
          <div className={styles.productModal}>
            ...
            <div className={styles.productModalBody}>
              ...
              <p className={styles.productModalDesc}>
                {activeProduct.description || 'Vendor-exclusive item, on-site only.'}
              </p>
              <div className={styles.productModalActions}>
                <button
                  className={`${styles.addToCartBtn} goldBTN`}
                ...
                <Link
                  href={`/events/vendor-shop/vendor?event=${eventId}&vendor=${activeProduct.vendor_id}`}
                  className={styles.viewVendorBtn}
                >
                  Visit stall
                </Link>
              </div>
          ...
          <aside className={styles.cartDrawer}>
            ...
            <div className={styles.cartBody}>
              {cart.length === 0 ? (
                <p className={styles.cartEmpty}>
                  Your cart is empty. Add a product from any vendor stall.
                </p>
              ) : (
                      ...
                      <div className={styles.cartItemBody}>
                        ...
                        <div className={styles.cartItemFoot}>
                          ...
                          <span className={styles.cartItemPrice}>
                            ₦{(item.qty * (item.price_ngn || item.price * 1000 || 0)).toLocaleString()}
                          </span>
                        </div>
                      </div>
                      <button
            ...
            <div className={styles.cartFooter}>
              ...
              <button
                ...
                onClick={() => {
                  alert('Checkout demo: items reserved at vendor booths. Pay on pickup with V-ENT wallet.');
                  clearCart();
                  setCartOpen(false);
                }}
                type="button"
              >
                <FaCheckCircle /> Reserve at booths
              </button>
              {cart.length > 0 && (
                <button
    ...
    fallback={
      <div className={styles.pageContainer}>
        ...
        <main className={styles.mainContainer}>
          <Sidebar />
          <div className={styles.rightPaneContainer}>
            <p className={styles.stateText}>Loading vendor shop…</p>
          </div>
        </main>
        <BottomMenu />
      </div>
