    ...
    const fetchPost = async () => {
      ...
      try {
        ...
        if (data.status === 'success') {
          ...
          // Seed a few mock comments locally so the thread feels alive.
          setComments(seedComments(data.data.post));
        }
      } catch (err) {
                    ...
                    comments.map((c) => (
                      <div key={c.id} className={styles.commentCard}>
                        <div className={styles.commentAvatar}>
                          <Image
                            src={c.author?.avatar || 'https://i.pravatar.cc/100?img=12'}
...
const seedComments = (post) => {
  if (!post) return [];
  const count = Math.max(0, post.comments_count || 0);
  if (count === 0) return [];
  const samples = [
    'Big W. Pulled up.',
  ...
  return Array.from({ length: Math.min(count, 6) }).map((_, i) => ({
    id: `seed_cmt_${post.id}_${i}`,
    body: samples[i % samples.length],
    author: {
      id: `user_seed_${i}`,
      username: `commenter_${i + 1}`,
      full_name: `Commenter ${i + 1}`,
      avatar: `https://i.pravatar.cc/120?img=${(i + 25) % 70}`,
    },
